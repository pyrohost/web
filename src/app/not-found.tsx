...
const NotFound = () => {
	return (
		<div className="flex h-full w-full flex-1 items-center justify-center gap-12 p-8">
			<div className="flex flex-col gap-12 rounded-3xl border-[#ffffff18] border-[1px] bg-[#ffffff18] p-8 md:flex-row">
				<div className="flex max-w-sm flex-col gap-8 text-left">
					<h1 className="font-extrabold text-[32px] leading-[98%] tracking-[-0.07rem]">404</h1>
					<p>This page does not exist. Let&apos;s get you back to somewhere that does. And maybe play some Minecraft while you&apos;re at it.</p>
					<div className="flex flex-col gap-2">
						<Link className="flex w-fit items-center gap-2 fill-brand text-brand" rel="noopener noreferrer" target="_blank" href="/account">
							<div className="flex h-4 w-4 shrink-0">
								<Logomark />
							</div>
							Pyro Account
						</Link>
						<Link className="flex w-fit items-center gap-2 fill-brand text-brand" rel="noopener noreferrer" target="_blank" href="https://status.pyro.host">
							<div className="flex h-4 w-4 shrink-0">
								<Logomark />
							</div>
							Status Page
				...
				<img
					alt=""
					className="flex aspect-square w-full shrink-0 rounded-2xl object-cover md:w-64"
					height="256"
