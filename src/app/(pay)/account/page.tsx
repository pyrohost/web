	...
	return (
		<div className="container mx-auto flex flex-col gap-4 py-8">
			{searchParams?.error === "NoActiveSubscriptionsDiscordLinkedRole" && (
				<div
					className="relative flex rounded-full bg-red-800/20 px-4 py-3 text-red-400"
					role="alert"
				>
			...
			{searchParams?.success === "DiscordLinkedRole" && (
				...
				>
					...
					<span className="ml-0.5 block sm:inline">
						Check your roles on the Pyro Discord server!
					</span>
					<Link aria-label="Close alert" className="ml-auto" href="/account">
						<Cross1Icon className=" h-6 w-6" />
					</Link>
			...
			<Suspense
				fallback={<DashboardSkeletonSection title={"Account Information"} />}
			>
			...
			<Suspense
				fallback={<DashboardSkeletonSection title={"Active Subscriptions"} />}
			>
