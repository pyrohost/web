...
import React, { useMemo, useState, useRef, useEffect, DetailedHTMLProps, HTMLAttributes } from 'react';
...
const RenderIfVisible = ({
            ...
            const resizeObserver = new ResizeObserver((entries) => {
                ...
                setRootHeight((prev) => {
                    if (
                        ...
                        (resizeEntry?.contentRect.height && resizeEntry?.contentRect.height > prev!)
                    ) {
            ...
            const observer = new IntersectionObserver(
                (entries) => {
                    if (typeof window !== 'undefined' && window.requestIdleCallback) {
                        window.requestIdleCallback(() => setIsVisible(entries[0]?.isIntersecting ?? false), {
                            timeout: 600,
                        });
                    } else {
    ...
    const rootClasses = useMemo(() => `render-if-visible ${rootElementClass}`, [rootElementClass]);
    ...
    return React.createElement(
        ...
        <div
            ...
            style={
                { overflow: 'hidden', rootStyle } as DetailedHTMLProps<HTMLAttributes<HTMLDivElement>, HTMLDivElement>
            }
        >
